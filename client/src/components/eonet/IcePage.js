...
export default function IcePage() {
  ...
  const placeMarkers = (events) => {
    events.forEach(event => {
      const geometry = event.geometry[0]; // Take the first set of coordinates
      ...
      marker.setPopup(new maptilersdk.Popup()
        .setHTML(`<h3>${event.title}</h3>
                  ...
                  <p>Date: ${date}</p>
                  <p>ID: ${event.id}</p>
                  <p>Source: ${event.sources[0].url}</p>`)
      );
    });
  };
